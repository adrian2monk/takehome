	...
	<div class="flex flex-wrap my-5 md:mx-4 justify-around xl:max-w-5xl xl:mx-auto">
		<div class="overflow-hidden p-2">
			<span class="float-left inset-y-0 flex items-center pl-2 mr-3">
				<svg
					class="h-10 w-10 fill-slate-300"
		...
		<div
			class="md:border-0 md:drop-shadow-none drop-shadow border border-slate-300 rounded-md overflow-hidden mx-2 md:mt-0 mt-4 p-2 md:flex-none flex-auto"
		>
			<h3 class="text-sm font-medium text-slate-900">Account</h3>
			<p
				class="text-sm font-bold uppercase"
				class:text-sky-500={user.account === 'scholar'}
				class:text-green-500={user.account === 'breeder'}
			>
				{user.account}
			</p>
		</div>
		...
		>
			...
			<div class="flex items-center">
				...
				<p class="text-sm font-light text-green-500">
					{new Intl.NumberFormat('en-US', { maximumSignificantDigits: 3 }).format(user.total_erc20)}
					WETH
				</p>
			</div>
		</div>
		<div class="md:place-self-end md:m-0 overflow-hidden m-5">
			<h3 class="text-sm font-medium text-slate-400">
				Total NFTs <span class="text-slate-900">{user.total_nfts}</span>
			</h3>
		</div>
		<div class="md:place-self-end md:m-0 overflow-hidden m-5">
			<h3 class="text-sm font-medium text-slate-400">
				Total Transactions <span class="text-sky-500">{user.total_transactions}</span>
			</h3>
		</div>
	</div>
{/if}
{#if stats}
	...
	>
		{#each stats as tx}
			<section transition:fade class="flex rounded-md bg-white drop-shadow-md mt-6 p-2">
				<img
					class="h-10 w-10"
					src={assetsApi + `/${assetByType(tx.name)}.png`}
					alt="Market Place"
				/>
	...
	<div
		class="flex justify-around gap-4 mt-6 mx-2 lg:flex-row-reverse lg:justify-start lg:mx-4 xl:max-w-5xl xl:mx-auto"
	>
		<div class="flex items-center">
			<PaginatedSearch bind:id bind:count page={0}>
				<button class="px-4 py-2 rounded-md hover:bg-slate-100 font-medium text-sky-500"
					>First</button
				>
			</PaginatedSearch>
			<PaginatedSearch bind:id bind:count page={page - 1}>
				<button
					disabled={page - 1 < 0}
					class="px-4 py-2 rounded-md hover:bg-slate-100 font-medium text-sky-500"
				>
					<svg
						class="w-5 h-5 fill-sky-500"
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 320 512"
					>
						<!--! Font Awesome Pro 6.0.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
	...
	<table
		class="hidden border-collapse w-full table-fixed mx-2 mt-3 lg:table lg:mx-4 xl:max-w-5xl xl:mx-auto"
	>
		...
		<tbody>
			{#each txs as tx}
				<tr class="border-b hover:bg-slate-100">
					...
					<td class="p-6 text-sm text-slate-400 capitalize"
						>{formatDistanceToNow(tx.txs_date, { addSuffix: true })}</td
					>
					<td
						class="p-6 text-sm font-medium capitalize"
						class:text-green-500={tx.status === 'success'}
						class:text-red-500={tx.status === 'failure'}>{tx.status}</td
					>
					<td class="py-6 text-sm truncate">
						<a class="text-sky-600" href={`/address/${tx.to}`}>
							<img
								class="h-5 w-5 float-left"
								src={assetsApi + `/${assetByType(tx.txs_type)}.png`}
								alt="Market Place"
							/>
			...
			<section class="grid grid-cols-1 border-b p-2">
				...
				<div class="flex p-4 place-content-between">
					...
					<p class="text-sm text-slate-400 capitalize">
						{formatDistanceToNow(tx.txs_date, { addSuffix: true })}
					</p>
				</div>
				<div class="flex p-4 place-content-between">
					...
					<p
						...
						class:text-green-500={tx.status === 'success'}
						class:text-red-500={tx.status === 'failure'}
					>
						{tx.status}
					</p>
				</div>
